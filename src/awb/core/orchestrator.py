...
class Orchestrator:
    ...
    def _base_spec(self,role):
        a=self._agent(role); return a.provider if a and a.provider else self.workspace.manifest.runtime.default_provider
    ...
    def _provider(self,role,task:Task|None=None):
        if self.provider_override is not None: return self.provider_override
        spec=self._base_spec(role)
        if spec.kind!='openai' and self._should_escalate(role,task):
