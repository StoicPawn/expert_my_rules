    ...
    try:
        ...
        if isinstance(agents, list) and agents:
            ...
            if {'director', 'worker', 'reviewer', 'verifier'}.issubset(roles):
                # The planner controls role/instructions, but not model identity.
                # Re-attach the configured heterogeneous local providers here so
                # AI-generated workspaces cannot silently collapse to one model.
                manifest['agents'] = with_role_providers(valid)
