...
def _execute(path: Path, title: str, code: str, timeout_seconds: int, metadata: dict | None = None) -> dict:
    ...
    try:
        proc = subprocess.run([sys.executable, '-I', str(script)], cwd=run_dir, capture_output=True, text=True, timeout=timeout_seconds, env={'PATH': os.getenv('PATH', ''),'PYTHONUNBUFFERED':'1','HOME':'/tmp','LANG':os.getenv('LANG','C.UTF-8')})
        returncode = int(proc.returncode); stdout = proc.stdout[-100_000:]; stderr = proc.stderr[-100_000:]
    ...
    record = {'id':run_id,'workspace_id':path.name,'title':title,'code':code,'status':status,'returncode':returncode,'stdout':stdout,'stderr':stderr,'metadata':metadata or {},'created_at':started,'finished_at':_now(),'timeout_seconds':timeout_seconds}
    runs_dir = path / 'runs'; runs_dir.mkdir(parents=True, exist_ok=True)
...
def capabilities(): return {'python':sys.version.split()[0],'max_timeout_seconds':900,'persistent_workspaces':True,'actions':['run','create_workspace','list_runs','latest_context','publish_context']}

@lab_app.get('/api/workspaces', dependencies=[Depends(_require_api)])
def list_workspaces(): return _workspaces()

@lab_app.post('/api/workspaces', dependencies=[Depends(_require_api)])
async def create_workspace(request: Request):
    data = await request.json(); name = str(data.get('name') or 'Research workspace').strip(); wid = f'LAB-{uuid.uuid4().hex[:10].upper()}'; path = _root() / 'workspaces' / wid; path.mkdir(parents=True, exist_ok=False)
    record = {'id':wid,'name':name,'description':str(data.get('description') or ''),'source_project':str(data.get('source_project') or 'shared'),'metadata':dict(data.get('metadata') or {}),'created_at':_now()}
    (path/'workspace.json').write_text(json.dumps(record,indent=2,ensure_ascii=False),encoding='utf-8'); return record

@lab_app.get('/api/workspaces/{workspace_id}/runs', dependencies=[Depends(_require_api)])
def list_runs(workspace_id: str): return _runs(_workspace_dir(workspace_id))
...
async def api_run(workspace_id: str, request: Request):
    data = await request.json(); code = str(data.get('code') or '')
    if not code.strip(): raise HTTPException(400, 'code is required')
    return _execute(_workspace_dir(workspace_id), str(data.get('title') or 'Experiment'), code, int(data.get('timeout_seconds') or 320), dict(data.get('metadata') or {}))


@lab_app.get('/', response_class=HTMLResponse)
def index(request: Request, project: str = ''):
    spaces = _workspaces(); selected = next((x for x in spaces if (x.get('metadata') or {}).get('project_key') == project), spaces[0] if spaces else None)
    cards = ''.join(f"<option value='{esc(w['id'])}' {'selected' if selected and w['id']==selected['id'] else ''}>{esc(w['name'])}</option>" for w in spaces)
    recent = _runs(_workspace_dir(selected['id']))[:8] if selected else []
    recent_html = ''.join(f"<div class='task {'done' if r.get('status')=='complete' else 'bad'}'><div class='split'><b>{esc(r.get('title'))}</b><span class='badge'>{esc(r.get('status'))}</span></div><pre class='output' style='max-height:160px'>{esc(r.get('stdout') or r.get('stderr') or 'No output')}</pre></div>" for r in recent)
    project_link = _cross_link(request,'AWB_PORT',8100,f'/project/{project}') if project else _cross_link(request,'AWB_PORT',8100)
    body = f"""
<div class='topbar'><div><h1>Research Lab</h1><p class='sub'>Spazio separato per esperimenti Python riproducibili e contesto condiviso con Expert My Rules.</p></div><div class='nav'><a href='{_cross_link(request,'AWB_PORT',8100)}'>← Progetti</a><a href='{_cross_link(request,'AWB_OBSERVER_PORT',8101)}'>Dashboard live</a>{f"<a href='{project_link}'>Setup progetto</a>" if project else ''}</div></div>
...
def ui_workspace(name: str = Form(...), project_key: str = Form('')):
    wid = f'LAB-{uuid.uuid4().hex[:10].upper()}'; path = _root() / 'workspaces' / wid; path.mkdir(parents=True, exist_ok=False)
    ...
    (path/'workspace.json').write_text(json.dumps(record,indent=2,ensure_ascii=False),encoding='utf-8'); return RedirectResponse(f'/?project={project_key}',303)

@lab_app.post('/ui/run')
def ui_run(workspace_id: str = Form(...), title: str = Form('Experiment'), code: str = Form(...), timeout_seconds: int = Form(320)):
    if not code.strip(): raise HTTPException(400,'code is required')
    _execute(_workspace_dir(workspace_id), title, code, timeout_seconds, {'caller':'research_lab_ui'}); return RedirectResponse('/',303)
