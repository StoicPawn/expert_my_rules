...
def shell(title: str, body: str, *, active: str = 'setup', extra_head: str = '', extra_script: str = '') -> str:
    setup_cls = 'active' if active == 'setup' else ''
    dash_cls = 'active' if active == 'dashboard' else ''
    lab_cls = 'active' if active == 'lab' else ''
    return f"""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>{esc(title)}</title><style>{BASE_CSS}</style>{extra_head}</head><body><div class='shell'><div class='topbar'><div class='brand'>Expert My Rules</div><nav class='nav'><a class='{setup_cls}' href='http://'+''>Setup</a></nav></div>{body}</div><script>
(function(){{
  document.querySelectorAll('[data-tab]').forEach(function(btn){{btn.addEventListener('click',function(){{var name=btn.dataset.tab;document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===btn));document.querySelectorAll('.pane').forEach(x=>x.classList.toggle('active',x.dataset.pane===name));}})}});
}})();
{extra_script}
</script></body></html>""".replace("<nav class='nav'><a class='"+setup_cls+"' href='http://'+''>Setup</a></nav>", "")
