...
def index(request: Request):
    ...
    for root in sorted(base_dir().iterdir()):
        ...
        try:
            ...
            cards.append(f"<a class='card linkcard' href='/project/{esc(root.name)}'><div class='split'><div><h2>{esc(state['name'])}</h2><div class='small muted'>{esc(current['title'] if current else 'Nessun task in esecuzione')}</div></div><span class='badge {'live' if status=='RUNNING' else ''}'>{esc(status)}</span></div><div class='meter'><i style='width:{state['progress_percent']}%'></i></div><div class='small muted' style='margin-top:7px'>{state['progress_percent']}% · {state['passed_gates']}/{state['total_gates']} gate</div></a>")
        except Exception:
            continue
    body = f"""<div class='topbar'><div><h1>Dashboard live</h1><p class='sub'>Solo ciò che serve per capire cosa sta facendo il sistema adesso.</p></div><div class='nav'><a href='{_cross_link(request,'AWB_PORT',8100)}'>Setup</a><a href='{_cross_link(request,'AWB_LAB_PORT',8102)}'>Research Lab</a></div></div>{''.join(cards) if cards else "<div class='empty'>Nessun progetto attivo.</div>"}"""
...
def project(request: Request, project: str):
    ...
    ws = load_workspace(root)
    body = f"""
<div class='topbar'><div><a class='small muted' href='/'>← dashboard</a><h1>{esc(ws.manifest.name)}</h1><p class='sub' id='northstar'>{esc(ws.manifest.goal)}</p></div><div class='nav'><a href='{_cross_link(request,'AWB_PORT',8100,f'/project/{project}')}'>Setup</a><a href='{_cross_link(request,'AWB_LAB_PORT',8102,f'/?project={project}')}'>Research Lab</a></div></div>
<div class='grid'>
 <div class='col-8 card hero'><div class='split'><div><h2>Job in corso</h2><div id='jobTitle' style='font-size:22px;font-weight:800'>Caricamento…</div><div class='muted' id='jobDetail'></div></div><span id='jobBadge' class='badge'>—</span></div><div class='meter' style='margin-top:14px'><i id='progressBar' style='width:0%'></i></div><div class='small muted' id='progressText' style='margin-top:6px'></div></div>
 <div class='col-4 card'><h2>ACEPC</h2><div class='kpis' style='grid-template-columns:repeat(2,minmax(0,1fr))'><div class='kpi'><span>CPU</span><b id='cpu'>—</b></div><div class='kpi'><span>RAM</span><b id='ram'>—</b></div><div class='kpi'><span>Modello</span><b id='model' style='font-size:13px'>—</b></div><div class='kpi'><span>API mese</span><b id='apiCost' style='font-size:16px'>—</b></div></div></div>
 <div class='col-7 card'><div class='split'><div><h2>Cosa sta producendo</h2><p class='small muted'>Clicca il task o apri il dettaglio per vedere output, review e strategia.</p></div><span id='liveState' class='badge'>idle</span></div><div id='currentTask' class='empty'>Nessun task attivo.</div><button id='toggleOutput' class='secondary' type='button'>Mostra output live / ultimo tentativo</button><pre id='output' class='output hide'></pre></div>
...
function render(s){{latest=s;document.getElementById('northstar').textContent=s.goal;const j=s.job||{{status:'NOT STARTED',detail:''}};document.getElementById('jobBadge').textContent=j.status;document.getElementById('jobBadge').className='badge '+(j.status==='RUNNING'?'live':j.status==='FAILED'?'bad':'');document.getElementById('jobTitle').textContent=s.current_task?s.current_task.title:(j.status==='COMPLETE'?'North Star completata':'Nessun task in esecuzione');document.getElementById('jobDetail').textContent=(j.detail||'')+(j.steps_done!=null?' · '+j.steps_done+' iterazioni':'');document.getElementById('progressBar').style.width=pct(s.progress_percent)+'%';document.getElementById('progressText').textContent=s.progress_percent+'% · '+s.passed_gates+'/'+s.total_gates+' gate · '+s.done_tasks+'/'+s.total_tasks+' task chiusi';const r=s.resources||{{}};document.getElementById('cpu').textContent=r.cpu_percent==null?'sampling…':Math.round(r.cpu_percent)+'%';document.getElementById('ram').textContent=r.ram_percent==null?'—':Math.round(r.ram_percent)+'%';document.getElementById('model').textContent=r.model||'—';const b=s.budget||{{}};document.getElementById('apiCost').textContent='€'+Number(b.monthly_spent_eur||0).toFixed(3)+' / €'+Number(b.monthly_budget_eur||0).toFixed(2);const p=s.runtime_progress||{{}};document.getElementById('liveState').textContent=p.state||'idle';const c=s.current_task;if(c){{document.getElementById('currentTask').className='task live';document.getElementById('currentTask').innerHTML='<b>'+h(c.title)+'</b><div class="small muted">'+h(c.description)+'</div><div class="small muted">scientific attempts '+c.scientific_attempts+' · technical failures '+c.technical_failures+'</div>'}}else{{document.getElementById('currentTask').className='empty';document.getElementById('currentTask').textContent='Nessun task attivo.'}}const liveTail=p.visible_tail||'';const a=s.latest_attempt||{{}};let out=liveTail;if(!out&&a.artifact)out=a.artifact;if(!out&&a.review)out='REVIEW\n'+JSON.stringify(a.review,null,2)+'\n\nVERIFICATION\n'+JSON.stringify(a.verification||{{}},null,2);document.getElementById('output').textContent=out||'Nessun output visibile ancora.';document.getElementById('gates').innerHTML=(s.gates||[]).map(g=>'<div class="task '+(g.passed?'done':'')+'"><div class="split"><b>'+h(g.id.replaceAll('_',' '))+'</b><span class="badge '+(g.passed?'ok':'warn')+'">'+(g.passed?'PASS':'OPEN')+'</span></div><div class="small muted">'+h(g.description)+'</div></div>').join('');document.getElementById('tasks').innerHTML=(s.tasks||[]).map(t=>'<div class="task '+(t.status==='DONE'?'done':t.status==='IN_PROGRESS'?'live':(t.status==='ERROR'||t.status==='BLOCKED')?'bad':'')+'"><div class="split"><b>'+h(t.title)+'</b><span class="badge">'+h(t.status)+'</span></div><div class="small muted">'+h(t.description)+'</div>'+(t.next_strategy?'<div class="small"><b>Next:</b> '+h(t.next_strategy)+'</div>':'')+'</div>').join('')||'<div class="empty">Nessun task.</div>';document.getElementById('events').innerHTML=(s.events||[]).slice(0,12).map(e=>'<div class="event"><b>'+h(e.kind.replaceAll('_',' '))+'</b><span>'+h(e.task_id||'project')+' · '+h(e.ts||'')+'</span></div>').join('');}}
async function tick(){{try{{const r=await fetch(endpoint,{{cache:'no-store'}});if(r.ok)render(await r.json());}}catch(e){{}}}}
document.getElementById('toggleOutput').addEventListener('click',()=>document.getElementById('output').classList.toggle('hide'));tick();setInterval(tick,2000);
"""
