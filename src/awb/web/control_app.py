...
from awb.core.models import ComputeNodeSpec, JobStatus, ModelRouteSpec, Task
...
from awb.web.app import ACTIVE, EXECUTOR, _start, base_dir, slug
...
def _project_body(request: Request, project: str) -> str:
    ...
    node_options = ''.join(f"<option value='{esc(n.id)}'>{esc(n.id)} · {esc(n.kind)}</option>" for n in ws.manifest.runtime.compute_nodes)
    role_cards = []
...
<div class='tabs'><button class='tab active' data-tab='north'>North Star</button><button class='tab' data-tab='team'>Team & modelli</button><button class='tab' data-tab='plan'>Piano</button><button class='tab' data-tab='api'>API & costi</button><button class='tab' data-tab='runtime'>Avvio</button></div>
...
<section class='pane' data-pane='plan'><div class='grid'><div class='col-8 card'><div class='split'><div><h2>Task verso la North Star</h2><p class='small muted'>Il sistema crea il piano iniziale; il Director può aggiungere/decomporre task durante il lavoro.</p></div>{badge(f'{len(tasks)} task')}</div>{''.join(task_cards)}</div><div class='col-4 card'><h2>Aggiungi task</h2><form method='post' action='/project/{esc(project)}/task'><label>Titolo</label><input name='title' required><label>Descrizione</label><textarea name='description'></textarea><label>Priorità</label><input name='priority' type='number' step='0.1' value='10'><button>Aggiungi al piano</button></form></div></div></section>
...
<section class='pane' data-pane='runtime'><div class='grid'><div class='col-7 card hero'><h2>Job autonomo</h2><p>Stato: <b>{esc(job_status)}</b>{' · '+str(job['steps_done'])+' iterazioni' if job else ''}</p>{controls}</div><div class='col-5 card'><h2>Collegamenti</h2><p><a href='{_cross_link(request,'AWB_OBSERVER_PORT',8101,f'/project/{project}')}'>Apri Dashboard live →</a></p><p><a href='{_cross_link(request,'AWB_LAB_PORT',8102,f'/?project={project}')}'>Apri Research Lab →</a></p><p class='small muted'>Setup, monitoraggio e laboratorio sono separati per evitare un'unica pagina tecnica troppo densa.</p></div></div></section>
"""
